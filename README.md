...
![升级标记](docs/assets/preview-update-badge.png)
![展开态升级标记](docs/assets/preview-update-panel.png)
![托盘升级菜单](docs/assets/preview-update-tray-menu.png)
...
- 发现新版本时，折叠态和展开态都会显示可点击的“升级”标记。
- 托盘图标会提示有新版本，右键托盘菜单提供“升级到 vX.Y.Z”入口。
- 构建、安装和卸载脚本减少容易被安全软件误报的写法，保持脚本行为更透明、可审计。

## 快速启动

1. 从 GitHub Release 下载 `codex-bubble-setup-v0.1.8.exe`。
...
软件启动后会安静检查一次 GitHub 最新 Release，也可以右键悬浮球选择“检查更新”手动检查。

如果发现新版本，折叠态和展开态都会显示“升级”标记，点击后打开新版安装器下载地址；系统托盘图标也会提示有新版本，右键托盘可以直接升级。下载后运行安装器即可覆盖升级，安装器会先退出旧版悬浮球和后台同步器，再安装新版。
